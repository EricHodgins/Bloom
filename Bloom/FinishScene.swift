...
class FinishScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let flareAction = SKAction.customAction(withDuration: 300) { (node, elapsed) in
            ...
            //sunFlare.inputOrigin = CIVector(x: 0 + (elapsed*10), y: (view.frame.height - 100) + elapsed)
            sunFlare.inputOrigin = CIVector(x: 0 + (elapsed*20), y: (view.frame.height*0) + elapsed)
            sunFlare.inputColor = CIVector(x: (elapsed * 20)/255, y: 45/255, z: 135/255)
